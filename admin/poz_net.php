<?php
	...
	// AND date_format(synag_reje.bashlayan_wagty, '%Y-%m-%d')='$sene'
	while ($row_toparcha=mysqli_fetch_array($query_toparcha)) {
		...
		$query_ders=mysqli_query($con, "SELECT ders_atlary.ady AS ders_ady, ders_maglumat.id AS ders_id FROM ders_maglumat, ders_atlary, bellenen_jogap_pdf WHERE ders_maglumat.ders_id=ders_atlary.id AND ders_maglumat.ara_syn_deg='1' AND ders_maglumat.toparcha_id='$toparcha_id' AND bellenen_jogap_pdf.sene='$sene' AND bellenen_jogap_pdf.ders_id=ders_maglumat.id LIMIT 0,1");
					...
					<tr>
						...
						while ($row_ders=mysqli_fetch_array($query_ders)) {
							?>
								...
								<th class="rot"><i class='fas fa-trash'></i></th>
							<?php
						}
						?>
					</tr>
							...
							<tr>
								...
								while ($row_ders=mysqli_fetch_array($query_ders)){
									...
									?>
										...
										<td><button data-id="<?php echo $talyp_id; ?>" class='ochurmek btn-sm btn-danger'><i class='fas fa-trash'></i></button></td>
									<?php
								}
								?>
							</tr>
