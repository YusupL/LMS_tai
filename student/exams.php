                    ...
                    while ($row=mysqli_fetch_array($query)) {
                        ...
                        <form method="POST" action="test.php">
                            <div class="card">
                                ...
                                <?php
                                    $query_pdf=mysqli_query($con, "SELECT * FROM bellenen_jogap_pdf WHERE talyp_id='$id_talyp' AND ders_id='$ders_id' AND aralyk_jemleme='$aj'");
                                    $count_pdf=mysqli_num_rows ($query_pdf);
                                    if ($count_pdf>0){
                                        while ($row_pdf=mysqli_fetch_array($query_pdf)) {
                                            $t_jogap=$row_pdf['bellenen_jogap'];
                                        }
                                        $m_jogap='000000000000000';
                                        $query_pdf_m=mysqli_query($con, "SELECT * FROM testler_pdf WHERE ders_id='$ders_id' AND aralyk_jemleme='$aj'");
                                        while ($row_pdf_m=mysqli_fetch_array($query_pdf_m)) {
                                            
                                            $m_jogap=deshifr_kichi($row_pdf_m['jogap']);
                                        }
                                        $d=0;
                                        $y=0;
                                        $color="";
                                        if ($hun_gor=='1'){
                                            ...
                                            $baha=$d/$ss*5;
                                            $baha=round($baha, 2);                                            
                                            if ($baha>=4.5) $color="#208C05";
                                            if (($baha>=3.5)&&($baha<4.5)) $color="#A8F940";
                                            if (($baha>=2.5)&&($baha<2.5)) $color="#E1DE0E";
                                            if ($baha<2.5) $color="#F73E28";
                                         } 
                                             elseif ($hun_gor=='2') {
                                        //     for ($i=0; $i<strlen($m_jogap); $i++){                                                
                                        //         if (($m_jogap[$i]==$t_jogap[$i])&&($i<45)) $d+=0.5;
                                        //         if (($m_jogap[$i]==$t_jogap[$i])&&($i<81)&&($i>=45)) $d++;
                                        //         if (($m_jogap[$i]==$t_jogap[$i])&&($i<109)&&($i>=81)) $d+=1.5;
                                ...
                                ?>
                                <input class="ders_ady" type="submit" name="sub_ders" <?php if (($count_pdf>0)||($count_test>0)||($count_chaklendirme>0)) echo "disabled='disabled'"; ?> value="<?php echo $row['ders_ady']; ?>">
                                <?php if (($count_pdf>0)||($count_test>0)){ ?>
                                        <span style="font-size: 22px;">-<span class="text-muted"><?php if ($hun_gor=='2') echo " (".$d."/".$ss.")"; ?></span> <span style="color: <?php if ($hun_gor=='1') echo $color; else echo "#208C05"; ?>;"><b><?php echo $baha; ?></b></span></span>
                                    <?php
                                    }
